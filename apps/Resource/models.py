...
class Resource(models.Model):
    ...
    # We achieve this contraint by using the models.PROTECT constraint.
    # https://docs.djangoproject.com/en/2.2/ref/models/fields/#django.db.models.PROTECT
    current_user = models.ForeignKey(User, on_delete=models.PROTECT, related_name='res_being_used')  #TODO: Change this to our User model.

    # FK to the User who is currently managing the device.
    # This user would have admin priveledges over the device.
    # TODO: Yet to be decided what these admin priveledges are (This would mostly allow the user to edit fields)
    # Like the 'current user' above we donot allow the Device Admin User object
    # to be deleted when he has Resources assgined to him.
    # Enforce the constraint.
    device_admin = models.ForeignKey(User, on_delete=models.PROTECT, related_name='res_being_managed') #TODO: change this to our user model.
    #TODO: Also check if we really need the 'related_name' fields for both the above 2 models.
    # The related_name allows us to fetch the resource list from the User model itself. Very useful as we would mostly need
    # to fetch these details per user. Might be better to let the ORM take care of this for us.
    ...
    # Resources which are 'Unassigned' and which are 'Assgined' but not 'Acknowledged' have
    ...
    # Every reasource will belong to an Organisation. This way we can have 
